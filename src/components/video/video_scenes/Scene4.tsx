...
export function Scene4() {
  ...
  return (
    <motion.div
      ...
      style={{ padding: 'clamp(16px, 6vw, 80px)' }}
    ...
    >
      {/* Left: Text */}
      <div className="shrink-0 pr-4" style={{ width: 'clamp(120px, 32%, 280px)' }}>
        <motion.div
          className="w-12 h-12 rounded-xl bg-green-500/20 flex items-center justify-center mb-4 border border-green-500/30"
        ...
        <motion.h2
          ...
          style={{ fontSize: 'clamp(18px, 3.5vw, 44px)' }}
          initial={{ y: 20, opacity: 0 }}
          animate={phase >= 1 ? { y: 0, opacity: 1 } : { y: 20, opacity: 0 }}
        ...
        <motion.div
          className="bg-green-600 rounded-xl p-3 flex items-center justify-center gap-2 w-max"
          initial={{ opacity: 0 }}
      ...
      <motion.div
        className="flex-1 bg-slate-900 border border-slate-700 rounded-xl overflow-hidden shadow-2xl flex flex-col"
        style={{ height: 'clamp(200px, 65vh, 520px)' }}
      ...
      >
        <div className="h-12 bg-slate-800 border-b border-slate-700 flex items-center px-4 justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm">Staff Schedule</span>
          </div>
        ...
        <div className="p-3 flex-1 flex flex-col gap-2 relative overflow-hidden">
          {[
            ...
            <motion.div
              key={i}
              className="bg-slate-800 rounded-lg px-3 py-2 flex items-center justify-between border border-slate-700 gap-2"
              initial={{ x: 50, opacity: 0 }}
              animate={phase >= 2 ? { x: 0, opacity: 1 } : { x: 50, opacity: 0 }}
              transition={{ duration: 0.5, delay: i * 0.12 }}
            >
              <div className="font-bold text-xs truncate w-[28%]">{row.name}</div>
              <div className="text-slate-400 text-xs truncate w-[28%]">{row.client}</div>
              <div className="text-slate-400 font-mono text-xs hidden sm:block w-[22%]">{row.time}</div>
              <div className={`px-2 py-0.5 rounded-full text-xs text-center font-bold shrink-0 ${row.color}`}>
                {row.status}
              </div>
            </motion.div>
          ))}
          ...
          <AnimatePresence>
            {phase >= 3 && (
              ...
              >
                <motion.div
                  className="bg-green-600 rounded-full p-4 mb-3"
                  initial={{ scale: 0 }}
                ...
                <h3 className="text-xl font-bold">Payroll.csv Generated</h3>
                <p className="text-slate-300 mt-1 text-sm">Ready for QuickBooks</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
