...
type AdminTab = 'schedule' | 'weekview' | 'newvisit' | 'clients' | 'caregivers' | 'payroll' | 'alerts' | 'invoices' | 'billing' | 'config' | 'exceptions' | 'completed' | 'client-history' | 'caregiver-history' | 'subscription' | 'users' | 'audit-log'| 'help';  
...
function ClientsTab({ onClientClick }: { onClientClick: (c: { id: number; name: string; address: string | null }) => void }) {
  ...
  const [clientImportResult, setClientImportResult] = useState<string | null>(null);
      ...
      {showClientImport && (
        <Card title="Import Clients from CSV">
          ...
          <input
            ...
            onChange={handleClientCsvFile}
            className="text-sm text-slate-600 mb-3 block"
          />
              ...
              <div className="overflow-x-auto mb-3 max-h-48 overflow-y-auto border rounded-lg">
                <table className="min-w-full text-xs">
                  <thead className="bg-slate-50 sticky top-0">
                    <tr>
                      {['Name','Address','Payer Type','Notes'].map(h => (
