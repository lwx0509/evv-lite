  ...
  static async processWebhook(payload: Buffer, signature: string): Promise<void> {
    ...
    switch (event.type) {
      ...
      case 'product.deleted':
        await pool.query(
          `UPDATE stripe.products SET updated_at=NOW() WHERE id=$1`,
          [data.id]
        );
        break;
      ...
      case 'price.deleted':
        await pool.query(
          `UPDATE stripe.prices SET updated_at=NOW() WHERE id=$1`,
          [data.id]
        );
        break;
      ...
      case 'customer.subscription.deleted':
        await pool.query(
          `UPDATE stripe.subscriptions SET status='canceled', updated_at=NOW() WHERE id=$1`,
          [data.id]
        );
        break;
