...
router.get('/billing/plans', async (_req, res) => {
    try {
        const rows = await storage_1.storage.listProductsWithPrices();
...
router.get('/billing/subscription', async (req, res) => {
    try {
        ...
        const info = await storage_1.storage.getAgencyStripeInfo(agencyId);
        if (!info?.stripe_customer_id) {
            return res.json({ subscription: null });
        }
        const sub = await storage_1.storage.getSubscriptionByCustomerId(info.stripe_customer_id);
...
router.post('/billing/checkout', async (req, res) => {
    try {
        ...
        let info = await storage_1.storage.getAgencyStripeInfo(agencyId);
